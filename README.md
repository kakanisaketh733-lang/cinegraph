...
Seed data: **40 well-known films across 10 directors and 108 credited actors**, hand-picked so the graph is genuinely connected — several Christopher Nolan/Michael Caine films, the Scorsese/DiCaprio run, the Coppola/Robert Duvall pairing, and actors who cross between directors (DiCaprio, Matt Damon, Harrison Ford, Sigourney Weaver) — enough overlap that recommendations, paths, and hidden-co-star queries actually turn up interesting results instead of empty ones.
...
### Regular company (`routes/company.js`) — aggregation over a 2-hop pattern
...
├── backend/
│   ├── db/
│   │   └── neo4j.js          # driver singleton + connection status helpers
│   ├── routes/
│   │   ├── health.js         # GET /api/health
│   │   ├── explore.js        # GET /api/movies, /api/movies/:title,
│   │   │                     #     /api/people, /api/people/:name, /api/genres
│   │   ├── recommend.js      # GET /api/recommend/:name
│   │   ├── path.js           # GET /api/path?from=&to=
│   │   ├── collaborators.js  # GET /api/collaborators/:name
│   │   └── company.js        # GET /api/company?director=&minCollaborations=
│   ├── seed/
│   │   ├── data.js           # the 40-movie dataset
│   │   └── seed.js           # constraints + idempotent MERGE-based loader
│   ├── .env.example
│   ├── package.json
│   └── server.js             # Express app: mounts API, serves frontend, SPA fallback
├── frontend/
│   ├── index.html            # 5-tab shell: Explore / Recommend / Six Degrees /
│   │                         #   Hidden Co-Stars / Regular Company
│   ├── style.css             # cinema-marquee theme, no framework
│   └── app.js                # vanilla JS: fetch calls + DOM rendering per tab
├── .gitignore
└── README.md
```
...
|---|---|---|
| GET | `/api/health` | `{ status, database, error, timestamp }` — connection status |
| GET | `/api/movies?search=&genre=` | List movies, optionally filtered |
| GET | `/api/movies/:title` | Single movie with full cast/crew/genres |
| GET | `/api/people?search=` | List people with acting/directing credit counts |
| GET | `/api/people/:name` | Single person's filmography |
| GET | `/api/genres` | List genres with movie counts |
| GET | `/api/recommend/:name` | Movies recommended for a person by genre overlap |
| GET | `/api/path?from=&to=` | Shortest ACTED_IN path between two people |
| GET | `/api/collaborators/:name` | People sharing a co-star but never cast together |
| GET | `/api/company?director=&minCollaborations=` | Director → actor collaboration counts |

Every route returns JSON. Errors come back as `{ "error": "message" }` with an appropriate status code (400 for bad input, 404 for not found, 500 for anything else, including a lost database connection).
